...
const HostVideo = () => {
  ...
  // Track uploads per contest for host limit
  const [contestUploadCounts, setContestUploadCounts] = useState({});
  ...
  const onSubmit = async (data) => {
    // Enforce 5 video limit per contest
    const currentCount = contestUploadCounts[data.contestTitle] || 0;
    if (currentCount >= 5) {
      setMessage({ type: 'error', text: 'You cannot upload more than 5 videos for a single contest.' });
      return;
    }
    ...
    try {
      ...
      // Increment count for this contest
      setContestUploadCounts(prev => ({
        ...prev,
        [data.contestTitle]: (prev[data.contestTitle] || 0) + 1
      }));

      setMessage({ type: 'success', text: `Video added successfully! (${currentCount + 1}/5 for this contest)` });
      reset();
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <div className="form-control w-full">
              ...
              <input
                ...
                className={`input input-bordered w-full ${errors.contestTitle ? 'input-error' : ''}`}
                disabled={uploading}
              />
              {errors.contestTitle && <span className="label-text-alt text-error mt-1">{errors.contestTitle.message}</span>}
            </div>
