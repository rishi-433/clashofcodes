...
function AdminUpdateForm() {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      const errMsg = error.response?.data?.message || (typeof error.response?.data === 'string' ? error.response.data : error.message);
      alert(`Error: ${errMsg}`);
    }
  };
  ...
  useEffect(() => {
    const fetchProblem = async () => {
      try {
        ...
        // Transform the data to match the form schema if necessary
        reset(data);
      } catch (err) {
        console.error(err);
          ...
          <div className="space-y-4">
            ...
            <div className="form-control">
              ...
              <input
                type="url"
                {...register('videoLink')}
                className={`input input-bordered ${errors.videoLink ? 'input-error' : ''}`}
                placeholder="e.g. https://www.youtube.com/watch?v=..."
              />
              {errors.videoLink && (
                <span className="text-error">{errors.videoLink.message}</span>
              )}
            </div>
          </div>
        </div>
            ...
            {startCodeFields.map((field, index) => (
              <div key={field.id} className="space-y-2">
                <h3 className="font-medium">
                  {referenceSolutionFields[index]?.language || field.language}
                </h3>
